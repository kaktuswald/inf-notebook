...
class GuiApi():
    ...
    def get_images(self, event: Event):
        ...
        if images[key] is None:
            image = load_image(dc.images_resultjudges_basepath, key)
    ...
    def get_recognitionresult(self, event: Event):
        ...
        result = {
            'pgreat': recog.Result.Judges.get_pgreat(image_np),
            'great': recog.Result.Judges.get_great(image_np),
            'good': recog.Result.Judges.get_good(image_np),
            'bad': recog.Result.Judges.get_bad(image_np),
            'poor': recog.Result.Judges.get_poor(image_np),
        }
    ...
    def delete_keyandlabel(self, event: Event):
        ...
        filepath = join(dc.images_resultjudges_basepath, key)
        if isfile(filepath):
            remove(filepath)
            del images[key]
    
    def save_labels(self):
        with open(dc.label_resultjudges_filepath, 'w') as f:
...
if __name__ == '__main__':
    if exists(dc.label_resultjudges_filepath):
        with open(dc.label_resultjudges_filepath) as f:
            labels = load(f)
    
    for filepath in glob(join(dc.images_resultjudges_basepath, '*')):
    ...
    window.show('web_manage/annotation_resultjudges.html')
