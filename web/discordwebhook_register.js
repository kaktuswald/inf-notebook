...
function set_inputdatetimes() {
  ...
  const startdtstring = startdt.toISOString().slice(0, 16);

  const enddt = new Date();
  enddt.setMinutes(enddt.getMinutes() - enddt.getTimezoneOffset());
  if(mode != DiscordwebhookModes.BATTLE)
    enddt.setDate(enddt.getDate() + 7);
  else
    enddt.setHours(enddt.getHours() + 8);
  const enddtstring = enddt.toISOString().slice(0, 16);

  const publishdt = new Date();
  publishdt.setMinutes(publishdt.getMinutes() - publishdt.getTimezoneOffset());
  const publishdtstring = publishdt.toISOString().slice(0, 16);
...
function get_input() {
  ...
  if(!checkinput('削除コード', deletecode, 40, false))
    return null;
  ...
  if(daydifference >= 7 * 9) {
    $('span#message').text(`${7*9}日以上の期間は設定できません。`);
    return null;
  }
...
function onchange_mode(e) {
  ...
  if(mode == DiscordwebhookModes.BATTLE) {
    $('input#date_start').prop('disabled', true);
    $('input#date_end').prop('disabled', true);
...
async function onclick_confirmregister(e) {
  ...
  const result = JSON.parse(await webui.discordwebhook_register(JSON.stringify(input)));
  ...
  $('span#message').text(`登録完了 ID: ${result.id}`);
  ...
  if($('input#check_downloadid').prop('checked')) {
    const text = [
      ...
      `削除コード: ${result.deletecode}`,
    ];
    const blob = new Blob([text.join('\n')], { text: 'text/plain'});
    const url = URL.createObjectURL(blob);
    const link = $('<a>')
      .attr('href', url)
      .attr('download', `ID-${result.id}.txt`)
    link[0].click();
  }
}
