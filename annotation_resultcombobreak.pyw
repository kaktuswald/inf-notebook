...
class GuiApi():
    ...
    def get_images(self, event: Event):
        ...
        if images[key] is None:
            image = load_image(dc.images_resultcombobreak_basepath, key)
    ...
    def get_recognitionresult(self, event: Event):
        ...
        result = {
            'combobreak': recog.Result.get_combobreak(image_np),
        }
    ...
    def delete_keyandlabel(self, event: Event):
        ...
        filepath = join(dc.images_resultcombobreak_basepath, key)
        if isfile(filepath):
            remove(filepath)
            del images[key]
    
    def save_labels(self):
        with open(dc.label_resultcombobreak_filepath, 'w') as f:
...
if __name__ == '__main__':
    if exists(dc.label_resultcombobreak_filepath):
        with open(dc.label_resultcombobreak_filepath) as f:
            labels = load(f)
    
    for filepath in glob(join(dc.images_resultcombobreak_basepath, '*')):
    ...
    window.show('web_manage/annotation_resultcombobreak.html')
